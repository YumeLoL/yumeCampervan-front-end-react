...
export const BookingsPage = () => {
  ...
  const memberId = JSON.parse(
    localStorage.getItem("yumeCamp_member") ?? "null"
  ).memberId;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getAllBookingsByMemberId(memberId, {
    ...
    <ContentWrap key={booking.bookingId}>
      <div className="flex justify-between px-4 py-5 sm:p-6">
        ...
        <div className="w-1/3 ml-4">
          <div
            className="w-34 h-44 bg-cover bg-center"
            style={{ backgroundImage: `url(${booking.imgUrl})` }}
          ></div>
          <div className="mt-4">
      ...
      <BookingContainer>
        ...
        <BookingDetails>
          ...
          {activeTab === RequestStatus.CONFIRMED && (
            <DetailBlock>{renderBookingDetails()}</DetailBlock>
          )}
